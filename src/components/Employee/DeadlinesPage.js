...
const DeadlinesPage = () => {
    ...
    const userId = useContext(UserContext).user.userDetails.id;

    useEffect(() => {
        // Fetch deadlines from the backend when the component mounts
        axios.get(`http://localhost:8080/tasks/get/${userId}`)
    ...
    const markAsComplete = async (id) => {
        try {
            ...
            await axios.patch(`http://localhost:8080/tasks/done/${id}`);
